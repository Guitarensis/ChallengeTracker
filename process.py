...
from globals import avg_et_output, avg_rt_output, avg_mph_output

name = 'pSilos Nitto Challenge Stats Tracker'

def process_packet(packet):
    global selected_car, cars, gear_ratio_data

    payload = str(packet[TCP].payload)

    # Check if the payload contains the expected GET request
    if "GET /gamecode152" in payload:
        ...
        if match:
            reaction_time = match.group(1)
            estimated_time = match.group(2)
            trap_speed = match.group(3)
            bracket_time = match.group(4)
            whine_data = match.group(5)
